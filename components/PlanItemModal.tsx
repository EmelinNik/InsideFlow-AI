...
export const PlanItemModal: React.FC<PlanItemModalProps> = ({
  ...
  useEffect(() => {
    if (item) {
        ...
        setIsWritingMode(false); // Reset writing mode on new item open
  ...
  const handleScriptCreated = (script: GeneratedScript) => {
      ...
      onSave(updatedItem);
      setIsWritingMode(false); // Close generator view
  };
  ...
  const modalWidthClass = 'max-w-2xl';
  
  // Calculate allowed archetypes for this platform
  const standardAllowed = PLATFORM_COMPATIBILITY[editedItem.platform] || [];
  // Ensure the CURRENT archetype is always in the list
  const displayArchetypes = standardAllowed.includes(editedItem.archetype) 
      ? standardAllowed 
      : [editedItem.archetype, ...standardAllowed];
      ...
      <div className={`bg-white w-full ${modalWidthClass} max-h-[90vh] rounded-2xl shadow-2xl flex flex-col relative z-10 animate-in fade-in zoom-in-95 duration-200 overflow-hidden transition-all ease-in-out`}>
          ...
          <div className="flex items-center gap-2">
            <div>
                <h2 className="text-xl font-bold text-slate-900">
                    {isArchived ? 'Создание поста для аналитики' : 'Редактирование поста'}
                </h2>
        ...
        <div className="flex-1 overflow-y-auto bg-slate-50/30">
              <div className="p-6 space-y-6">
                ...
                {/* CONTENT SECTION (GENERATOR OR TEXT) */}
                             ...
                             {isArchived ? (
                                ...
                                editedItem.generatedContent ? (
                                    ...
                                    <div className="text-center py-8 bg-slate-50 rounded-lg border border-slate-100 border-dashed">
                                        <p className="text-slate-400 text-sm mb-4">Текст еще не написан</p>
                                        <button 
                                            onClick={handleStartWriting}
                                            className="px-5 py-2 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700 transition-colors shadow-sm flex items-center gap-2 mx-auto text-sm"
                                        >
                                            <Wand2 size={16}/> Написать с ИИ
                                        </button>
                                    </div>
                                )
                            )}
